...
gdTest <- function(gdX=mtxX, gdY=mtxY, alpha=0.001, nIter=2000) {
    nVar <- ncol(gdX)  # Count the column of 1 as a predictor, not that it really matters . . . 
...
```{r}
graphFrame <- cbind(y0001[,1], y0003[,1], y0010[,1], y0030[,1], mtcars$mpg)
...
lines(x=1:nrow(graphFrame), y=graphFrame[,2], col="blue", lty=2)
lines(x=1:nrow(graphFrame), y=graphFrame[,3], col="purple", lty=2)
lines(x=1:nrow(graphFrame), y=graphFrame[,4], col="orange", lty=2)
```
