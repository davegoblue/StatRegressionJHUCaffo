...
nHiddenNodes <- 25
nFinalLabels <- 10
epsInit <- 0.12  # Selection for randomization of initial data

# First matrix needs [1 + nInputLayer] columns and nHiddenNodes rows
bpThetaOne <- matrix(data=runif(nHiddenNodes * (1 + nInputLayer), min=0, max=epsInit), 
                     nrow=nHiddenNodes, ncol=(1 + nInputLayer), byrow=FALSE
                     )

# Second matrix needs [1 + nHiddenNodes] columns and nFinalLables rows
bpThetaTwo <- matrix(data=runif(nFinalLabels * (1 + nHiddenNodes), min=0, max=epsInit), 
