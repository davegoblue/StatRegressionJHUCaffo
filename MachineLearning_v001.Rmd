...
Basic SVM uses a "linear kernel" (more about kernels in the next section).  These are available in R through the "kernlab" library.  We run kernlab::ksvm() using type "C-svc" (C classification) and kernel "vanilladot" (linear kernel):  
```{r}
library(kernlab)


# Run with C=1
svmLinear001 <- ksvm(svmX, svmY, type="C-svc", kernel="vanilladot", C=1)
plot(svmLinear001, data=svmX, main="SVM for C=1 (scaled inputs")
w001 <- colSums(coef(svmLinear001)[[1]] * svmX[alphaindex(svmLinear001)[[1]],])
b001 <- b(svmLinear001)

# Run with C=100
svmLinear100 <- ksvm(svmX, svmY, type="C-svc", kernel="vanilladot", C=100)
plot(svmLinear100, data=svmX, main="SVM for C=100 (scaled inputs")
w100 <- colSums(coef(svmLinear100)[[1]] * svmX[alphaindex(svmLinear100)[[1]],])
b100 <- b(svmLinear100)


plot(scale(svmX), col=1+svmY[,1], pch=19, cex=2, main="SVM Sample Data (Red for Y=1)")
abline(b001/w001[1], -w001[2]/w001[1], lty=1, lwd=2, col="blue")
abline(b100/w100[1], -w100[2]/w100[1], lty=1, lwd=2, col="purple")

```
  
This will need further research and exploration!  The concept is clear, but the implementation not so much.