...
* 10 runs of the gradient function take ~1.2 seconds  
...
* Extrapolating, the raw functions should take 4.3 * 0.6 + 1.2 = ~4 seconds  
...
* Extrapolating, the raw functions should take 2.8 * 0.6 + 1.2 = ~3 seconds  
...
* 10 runs of nnGradient takes ~2 seconds  
* Running optim() using BFGS for 150 iterations takes ~240 seconds, while making 257 calls to nnCost and 150 calls to nnGradient; these should only take ~55 seconds, so optim() is taking ~4x as long as expected  
* Running optim() using CG for 10 iterations takes ~4 seconds, while making 22 calls to nnCost and 11 calls to nnGradient; these should take ~4 seconds, suggesting very little inefficiency  
* Running optim() using CG for 50 iterations takes ~19 seconds, while making 107 calls to nnCost and 51 calls to nnGradient; these should take ~20 seconds, suggesting very little inefficiency  
* Running optim() using CG for 150 iterations takes ~55 seconds, while making 312 calls to nnCost and 151 calls to nnGradient; these should take ~60 seconds, suggesting very little inefficiency  
...
While the CG 150 is driving to 96.9% accuracy (1.3% higher classification error than BFGS 150).  And, we check that the starting cost is the same as the base case:  
```{r}
vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))  # Roll together bpThetaOne and bpThetaTwo
...
And, accuracy here is only 88%, suggestive that 50 iterations of CG in R is driving to a less optimal outcome than 50 iterations of CG in Octave (which hits ~95%).  So, perhaps there is still further tuning to explore?  