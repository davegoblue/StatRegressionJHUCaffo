...
nnGradient <- function(vecMatrix, nInputLayer, nHiddenNodes, nFinalLabels, X, y, lambda) {
    ...
    # Calculate the gradients using a for loop from 1-m
    for (intCtr in 1:m) {
        
        # Step 1 - Forward propagate this observation
        a1 <- matrix(data=c(1, X[intCtr,]), nrow=1)  # Take the intCtr row and add 1 in front (1x401)
        z2 <- a1 %*% t(useThetaOne)  # 1x401 %*% 401x25 = 1x25
        a2 <- cbind(1, sigmoidPlain(z2))  # Take sigmoid of z2, add 1 in front (1x26)
        z3 <- a2 %*% t(useThetaTwo)  # 1x26 %*% 26x10 = 1x10
        a3 <- sigmoidPlain(z3)  # Take sigmoid of z3, and this is a3
        
        # Step 2 - Calculate delta03
        delta_03 <- a3 - yPreds[intCtr,]  # This will be 1x10, same as a3
        
        # Step 3 - Calculate delta02
        delta_02 <- (delta_03 %*% useThetaTwo[,-1]) * sigmoidGradient(z2)  # (1x10 %*% 10x25) * 1x25 = 1x25
        
        # Step 4a - Accumulate Theta Gradeints
        gradThetaTwo <- gradThetaTwo + t(delta_03) %*% a2  # 10x26 + 10x1 %*% 1x26 = 10x26 (as desired)
        gradThetaOne <- gradThetaOne + t(delta_02) %*% a1  # 25x401 + 25x1 %*% 1x401 = 25x401 (as desired)
    }
