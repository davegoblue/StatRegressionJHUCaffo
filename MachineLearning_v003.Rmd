...
Logistic regression ("logit") is used to make binary classifications.  By convention, 1 often expresses the presence of something (malignancy, school acceptance, fraudulent transaction, etc.) while 0 is set to express the absence of that something.  The models work the same regardless of which state is called 1, of course.  
...
The LM appropriately responds to the new data by driving up the intercept (more of the data are positive, and making more positive predictions will drive up the overall accuracy).  But, this means we declare tumors of size 0.011 to have a 50% chance of being malignant, even though the closed form function has probability of malignancy at size 0.011 as ~20%.  
...
nnGradient <- function(vecMatrix, nInputLayer, nHiddenNodes, nFinalLabels, X, y, lambda) {
    ...
    # Step 4c - Add penalty term (lambda), except not for bias unit (first column)
    gradThetaTwo[, -1] <- gradThetaTwo[, -1] + (lambda/m) * useThetaTwo[, -1]
    gradThetaOne[, -1] <- gradThetaOne[, -1] + (lambda/m) * useThetaOne[, -1]
...
Still, it provides a good starting solution to a neural network with ~10,000 parameters for ~50,000 predictions (10 final nodes for each of the 5,000 observations) in ~4 minutes.  So, while it can likely be significantly further optimized, it is pretty cool to be able to do simple image identification (on a cleaned dataset consisting of numbers only) in not too much time.  
...
# Store the results for later optimization technique comparisons
nnListBFGS150 <- myList
```
  
The model is 98.8% accurate with identifying images (n.b., there was no out-of-sample test run, so this is an inflated accuracy) based on this simple neural network.  A very interesting exercise!  
  
###_Model Selection (Bias vs. Varaince)_  
A primary goal with prediction is to correctly classify out-of-sample observations, meaning data that was not used to train the model.  When applying a model to make out-of-sample predictions, there are three common sources of error:  
...
* The sum of the first n diagonals divided by the total sum(diagonals) describes the total variance captured by the first n principal components  
* X = U % * % diag(D) % * % t(V)  
* D = t(U) % * % X % * % V
...
In this example, the algorithm correctly flags 6 points in the CV data as anomalies, avoids classifying any normal data as anomalies, and fails to identify 2 actual anomalies.  Since these 2 anomalies are close to the center of the cloud, there may be a missing feature.  This would be ideal to explore further with a domain expert.  
...
###_Collaborative Filtering (Background)_  
Collaborative filtering is the solution.  Each user gives ratings for movies that they have watched, and the algorithm simultaneously solves for features by movie and thetas by user in a manner that minimizes costs.  The name comes from the fact that a large group of people are pooling their inputs in a manner that helps make better (lower cost/error) recommendations for everyone.  
  
* J = 0.5 * sum-over-(I,j):r(I,j)=1-of [ ( t(theta(j)) * x(i) - y(I, j) )^2 ] + ( 0.5 * lambda )  * sum( x(k)(i)^2 ) + ( 0.5 * lambda )  * sum( theta^2 )  
* There is no longer any need for a bias unit - the algorithm can create a bias unit if it likes  
* Each x is an n-dimensional vector, and each theta is an n-dimensional vector; the n features are picked by the model  
* Begin by initializing every theta and feature to a small random number (important for breaking symmetry)  
* x(k)(i) = x(k)(i) - alpha * { sum-over-j:r(I,j)=1-of [ ( t(theta(j)) * x(i) - y(I, j) ) * theta(k)(j) ] + lambda * x(k)(i) }  
* theta(k)(j) = theta(k)(j) - alpha * { sum-over-i:r(I,j)=1-of [ ( t(theta(j)) * x(i) - y(I, j) ) * x(k)(i) ] + lambda * theta(k)(j) }  
* After convergence, use t(theta) * x to predict how a user will rate a movie they have not yet seen  
  
While the final features will not have meaning per se, they can be used to group "similar" movies and "similar" users by finding small euclidean distances between movie features and/or user thetas  
  
###_Collaborative Filtering (Example Given Input Matrices)_  
An example is again drawn from the Ng Coursera Machine Learning files:  
```{r}
library(R.matlab, quietly=TRUE)
...
Next an algorithm is created to calculate the cost given the relevant thetas and features:  
```{r}
...
Then, the sample theta and features from Ng Coursera Machine learning are additionally read-in:  
```{r}
library(R.matlab, quietly=TRUE)
...
listMATData <- readMat(paste0(matPath, wgtData))
# Check that as expected - list with five items: $X, $Theta, $num.users, $num.movies, $num.features
str(listMATData)  
...
The cost function is then tested for 4 users, 5 movies, and 3 features to ensure it brings back ~22.22 given these base inputs.  
```{r}
...
# Confirm that runs OK for full dataset also
cofiCost(vecMatrix = c(as.vector(mtxX), as.vector(mtxTheta)), mtxY = movieRatings, 
         mtxR = mtxSparse, nUser=nUser, nMovie=nMovie, nFeature=nFeature, lambda=1.5
         )

```
  
So, provided that we have the proper inputs, we can calculate the associated costs.  The next step is to find the gradients and optimize the parameters.  
  
###_Collaborative Filtering (Example Solving for Input Matrices)_  
The process of simultaneously solving for theta and features is an example of low rank matrix factorization.  The rank of an mxn matrix is typically the smaller of m or n, defining the degree of linear independence that exists in the matrix.  However, when certain rows and/or columns are linear combinations of each other, the matrix is no longer considered to be "full rank".  Instead, the matrix is considered to be "rank deficient" and to have rank equal to the smaller of # linearly independent columns/rows.  Fortunately, gradient descent works fine even in a rank-deficient environment:  
```{r}
...
Then, we can initialize theta and X as small random numbers (break the symmetry), normalize the movie ratings, and call optim to find the solution:  
```{r, cache=TRUE}
useLambda=10

# Initialize mtxX and mtxTheta
mtxTheta <- matrix(rnorm(length(mtxTheta)), nrow=nrow(mtxTheta))
mtxX <- matrix(rnorm(length(mtxX)), nrow=nrow(mtxX))

# Normalize movie ratings (movieRatings is 1682 x 943, with each of the 1682 rows being a movie)
...
# cofiGrad <- function(vecMatrix, mtxY, mtxR, nUser, nMovie, nFeature, lambda)
# Initialize vecMatrix
vecMatrix = c(as.vector(mtxX), as.vector(mtxTheta))

startTime <- proc.time()
movList <- optim(vecMatrix, cofiCost, cofiGrad, mtxY=normRatings,
                 mtxR=mtxSparse, nUser=nUser, nMovie=nMovie, nFeature=nFeature,
                 lambda=useLambda, control=list(maxit=50), method="BFGS"
                 )
proc.time() - startTime

```
  
This runs much slower than the equivalent in Octave.  It may be helpful to explore optim() and/or a faster equivalent in more detail.  
  
###_Tuning R Optimization for Speed_  
It would be interesting to understand where the speed challenges occur within R.  For example, is the timing gap due to the underlying functions, or due to how optim() pieces everything together?  We check this with a few code snippets and proc.time():  
```{r, cache=TRUE}
# Print the outcomes from the above cached BFGS 50-iteration optimization
str(movList)

# Test the cost calculation function (10 iterations)
startTime <- proc.time()
for (intCtr in 1:10) {
    cofiCost(vecMatrix = c(as.vector(mtxX), as.vector(mtxTheta)), mtxY = movieRatings, 
...
# Test the gradient calculation function (10 iterations)
startTime <- proc.time()
for (intCtr in 1:10) {
    cofiGrad(vecMatrix = c(as.vector(mtxX), as.vector(mtxTheta)), mtxY = movieRatings, 
             mtxR = mtxSparse, nUser=nUser, nMovie=nMovie, nFeature=nFeature, lambda=10
             )
}
proc.time() - startTime

# Re-initialize thetas and re-create starting vecMatrix
mtxTheta <- matrix(rnorm(length(mtxTheta)), nrow=nrow(mtxTheta))
mtxX <- matrix(rnorm(length(mtxX)), nrow=nrow(mtxX))
vecMatrix = c(as.vector(mtxX), as.vector(mtxTheta))

# Run BFGS with 10 iterations for additional benchmarking
startTime <- proc.time()
movListBFGS010 <- optim(par=vecMatrix, fn=cofiCost, gr=cofiGrad, mtxY=normRatings,
                        mtxR=mtxSparse, nUser=nUser, nMovie=nMovie, nFeature=nFeature,
                        lambda=useLambda, control=list(maxit=10), method="BFGS"
                        )
proc.time() - startTime
...
The outcomes are intriguing and show that optim() is the primary time-sink:  
  
* 10 runs of the cost function take ~0.6 seconds  
* 10 runs of the gradient function take ~1.1 seconds  
* Total cost after 10 runs of BFGS is ~160k, roughly double the ~75k seen above at 50 iterations  
* 43 runs of cost and 10 runs of gradient made by way of BFGS in optim() take ~150 seconds  
* Extrapolating, the raw functions should take 4.3 * 0.6 + 1.1 = ~3.5 seconds  
  
So, it appears that the optim() function using BFGS is taking 30x-40x longer than calls to the underlying functions would suggest.  One potential reason is that BFGS is storing a matrix - see below for some comments from Details under ?optim:  
  
* Method "BFGS" is a quasi-Newton method (also known as a variable metric algorithm), specifically that published simultaneously in 1970 by Broyden, Fletcher, Goldfarb and Shanno. This uses function values and gradients to build up a picture of the surface to be optimized.  
* Method "CG" is a conjugate gradients method based on that by Fletcher and Reeves (1964) (but with the option of Polak-Ribiere or Beale-Sorenson updates). Conjugate gradient methods will generally be more fragile than the BFGS method, but as they do not store a matrix they may be successful in much larger optimization problems.  
  
As such, it would be interesting to test the performance using the conjugate gradients method, particularly since that will be a closer match to what is used in the Octave/MATLAB examples:  
```{r}
# Run for the same 10 ierations
# Re-initialize thetas and re-create starting vecMatrix
mtxTheta <- matrix(rnorm(length(mtxTheta)), nrow=nrow(mtxTheta))
mtxX <- matrix(rnorm(length(mtxX)), nrow=nrow(mtxX))
vecMatrix = c(as.vector(mtxX), as.vector(mtxTheta))
...
startTime <- proc.time()
movListCG010 <- optim(par=vecMatrix, fn=cofiCost, gr=cofiGrad, mtxY=normRatings,
                      mtxR=mtxSparse, nUser=nUser, nMovie=nMovie, nFeature=nFeature,
                      lambda=useLambda, control=list(maxit=10), method="CG"
                      )
proc.time() - startTime
...
The outcomes are highly suggestive that CG is an all-around better method for this specific example:  
  
* Total cost after 10 runs of CG is ~45k, roughly half the ~75k seen above at 50 iterations of BFGS  
* 28 runs of cost and 11 runs of gradient made by way of BFGS in optim() take ~2 seconds  
* Extrapolating, the raw functions should take 2.8 * 0.6 + 1.1 = ~2.5 seconds  
  
So, the CG method seems to take only as long to run as the underlying function calls, and converges to a lower cost with fewer iterations.  

The same process is then run using CG with additional iterations - once at 50 iterations, and once at 250 iterations.  The theta matrix is again re-initialized each time:  
```{r, cache=TRUE}

# Run for 50 ierations
# Re-initialize thetas and re-create starting vecMatrix
mtxTheta <- matrix(rnorm(length(mtxTheta)), nrow=nrow(mtxTheta))
mtxX <- matrix(rnorm(length(mtxX)), nrow=nrow(mtxX))
vecMatrix = c(as.vector(mtxX), as.vector(mtxTheta))
...
startTime <- proc.time()
movListCG050 <- optim(par=vecMatrix, fn=cofiCost, gr=cofiGrad, mtxY=normRatings,
                      mtxR=mtxSparse, nUser=nUser, nMovie=nMovie, nFeature=nFeature,
                      lambda=useLambda, control=list(maxit=50), method="CG"
                      )
proc.time() - startTime
...
startTime <- proc.time()
movListCG250 <- optim(par=vecMatrix, fn=cofiCost, gr=cofiGrad, mtxY=normRatings,
                      mtxR=mtxSparse, nUser=nUser, nMovie=nMovie, nFeature=nFeature,
...
The time consumed by the additional runs is roughly proportional to the increased number of iterations, suggesting that CG is a good method for this particular problem.  Further, there is almost no improvement in the overall cost when moving from 10 iterations CG -> 250 iterations CG, which suggests that a nice process might involve a smaller number of iterations but with more robust tuning of the optimal lambda parameter:  
  
```{r}
cofiValues <- c(movListBFGS010$value, movList$value, movListCG010$value, 
                movListCG050$value, movListCG250$value 
                )

plot(cofiValues/1000, xaxt="n", ylab="Cost in 000s (lambda=10)", main="Cost by Algorithm", 
     pch=19, col="blue", cex=2, xlab="", ylim=c(0, 50 * ceiling(max(cofiValues)/50000) )
     )
axis(1, at=1:5, labels=c("BFGS 10 iter", "BFGS 50 iter", "CG 10 iter", "CG 50 iter", "CG 250 iter"))

```
  
The moral seems clear - for this type of problem, use method="CG" for optimization!  
  
It would also be interesting to understand whether method="CG" could help with the neural network tuned previously.  The same code is run, with the method changed to CG for 10 iterations, 50 iterations, 150 iterations, and 300 iterations.  The goal is to see whether CG run-times and/or accuracies can be improved:  
```{r, cache=TRUE}
# Bring back the data that would have been used right at the start of the Neural Network
library(R.matlab, quietly=TRUE)
...
# Recycle the pre-existing bpThetaOne and bpThetaTwo
vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))  # Roll together bpThetaOne and bpThetaTwo


# Benchmark individual neural network functions
startTime <- proc.time()
for (intCtr in 1:10) {
    nnCost(vecMatrix=vecPass, nInputLayer=nInputLayer, nHiddenNodes=nHiddenNodes, 
           nFinalLabels=nFinalLabels, X=mtxX, y=mtxY, lambda=1
           )
}
proc.time() - startTime
...
startTime <- proc.time()
for (intCtr in 1:10) {
    nnGradient(vecMatrix=vecPass, nInputLayer=nInputLayer, nHiddenNodes=nHiddenNodes, 
               nFinalLabels=nFinalLabels, X=mtxX, y=mtxY, lambda=1
               )
}
proc.time() - startTime
...
startTime <- proc.time()
vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))  # Roll together bpThetaOne and bpThetaTwo
nnListCG010 <- optim(par=vecPass, fn=nnCost, gr=nnGradient, nInputLayer=nInputLayer,
                     nHiddenNodes=nHiddenNodes, nFinalLabels=nFinalLabels,
                     X=mtxX, y=mtxY, lambda=1, control=list(maxit=10), method="CG"
                     )
proc.time() - startTime

startTime <- proc.time()
vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))  # Roll together bpThetaOne and bpThetaTwo
nnListCG050 <- optim(par=vecPass, fn=nnCost, gr=nnGradient, nInputLayer=nInputLayer,
                     nHiddenNodes=nHiddenNodes, nFinalLabels=nFinalLabels,
                     X=mtxX, y=mtxY, lambda=1, control=list(maxit=50), method="CG"
                     )
proc.time() - startTime

startTime <- proc.time()
vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))  # Roll together bpThetaOne and bpThetaTwo
nnListCG150 <- optim(par=vecPass, fn=nnCost, gr=nnGradient, nInputLayer=nInputLayer,
                     nHiddenNodes=nHiddenNodes, nFinalLabels=nFinalLabels,
                     X=mtxX, y=mtxY, lambda=1, control=list(maxit=150), method="CG"
                     )
proc.time() - startTime

startTime <- proc.time()
vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))  # Roll together bpThetaOne and bpThetaTwo
nnListCG300 <- optim(par=vecPass, fn=nnCost, gr=nnGradient, nInputLayer=nInputLayer,
                     nHiddenNodes=nHiddenNodes, nFinalLabels=nFinalLabels,
                     X=mtxX, y=mtxY, lambda=1, control=list(maxit=300), method="CG"
                     )
proc.time() - startTime
...
The results are again interesting and show the enormous timing advantage for CG over BFGS.  For example:  
  
* 10 runs of nnCost takes ~1 seconds  
* 10 runs of nnGradient takes ~1.6 seconds  
* Running optim() using BFGS for 150 iterations takes ~240 seconds, while making 151 calls to nnCost and 150 calls to nnGradient; these should only take ~40 seconds, so optim() is taking ~6x as long as expected  
* Running optim() using CG for 10 iterations takes ~3.5 seconds, while making 21 calls to nnCost and 11 calls to nnGradient; these should take ~4 seconds, suggesting very little inefficiency  
* Running optim() using CG for 50 iterations takes ~18 seconds, while making 106 calls to nnCost and 51 calls to nnGradient; these should take ~19 seconds, suggesting very little inefficiency  
* Running optim() using CG for 150 iterations takes ~55 seconds, while making 322 calls to nnCost and 151 calls to nnGradient; these should take ~55 seconds, suggesting very little inefficiency  
* Running optim() using CG for 300 iterations takes ~120 seconds, while making 643 calls to nnCost and 301 calls to nnGradient; these should take ~115 seconds, suggesting very little inefficiency  
  
Additionally, the results are again plotted to see the estimated cost rates:  
```{r}
nnCostValues <- c(nnListBFGS150$value, nnListCG010$value, nnListCG050$value, 
                  nnListCG150$value, nnListCG300$value
                  )

plot(nnCostValues, xaxt="n", ylab="Cost (lambda=1)", main="Cost by Algorithm", 
     pch=19, col="blue", cex=2, xlab="", ylim=c(0, 1 * ceiling(max(nnCostValues)/1) )
     )
axis(1, at=1:5, labels=c("BFGS 150 iter", "CG 10 iter", "CG 50 iter", "CG 150 iter", "CG 300 iter"))

```
  
Interestingly, though, in this case CG is not converging to as good solution as had been observed in the BFGS modeling.  So, the algorithm may be faster but less accurate for this specific case.  
  
###_Comparing Accuracies_  
Further, we compare the accuracies driven by the varying neural network models.  First, a look at BFGS 150 (repeated from above):  
```{r}
keyCut <- (nInputLayer + 1) * nHiddenNodes
postBPThetaOne <- matrix(data=nnListBFGS150$par[1:keyCut], nrow=nHiddenNodes, byrow=FALSE)
postBPThetaTwo <- matrix(data=nnListBFGS150$par[(keyCut+1):length(nnListBFGS150$par)], 
                         nrow=nFinalLabels, byrow=FALSE
                         )

# Assess the optimized accuracies
predYBFGS150 <- forwardPredY(mtxX, postBPThetaOne, postBPThetaTwo)
plot(x=1:length(predYBFGS150), y=predYBFGS150, col = 1 + (mtxY[,1] != predYBFGS150), 
     main="Predictions vs Actual (BFGS 150)", ylab="Prediction (Red=Wrong)", xlab="Index"
     )
confusionMatrix(predYBFGS150, mtxY[,1])

# Print the post-optimization cost
nnCost(vecMatrix=nnListBFGS150$par, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)

# Plot the post-optimization gradients
plot(nnGradient(vecMatrix=nnListBFGS150$par, nInputLayer, nHiddenNodes, nFinalLabels, 
                X=mtxX, y=mtxY, lambda=1
                )
     )

```
  
So, the BFGS 150 is driving 98.8% accuracy.  Next, we look at the CG 150 outcomes:  
```{r}
keyCut <- (nInputLayer + 1) * nHiddenNodes
postBPThetaOne <- matrix(data=nnListCG150$par[1:keyCut], nrow=nHiddenNodes, byrow=FALSE)
postBPThetaTwo <- matrix(data=nnListCG150$par[(keyCut+1):length(nnListCG150$par)], 
                         nrow=nFinalLabels, byrow=FALSE
                         )

# Assess the optimized accuracies
predYCG150 <- forwardPredY(mtxX, postBPThetaOne, postBPThetaTwo)
plot(x=1:length(predYCG150), y=predYCG150, col = 1 + (mtxY[,1] != predYCG150), 
     main="Predictions vs Actual (CG 150)", ylab="Prediction (Red=Wrong)", xlab="Index"
     )
confusionMatrix(predYCG150, mtxY[,1])

# Print the post-optimization cost
nnCost(vecMatrix=nnListCG150$par, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)

# Plot the post-optimization gradients
plot(nnGradient(vecMatrix=nnListCG150$par, nInputLayer, nHiddenNodes, nFinalLabels, 
                X=mtxX, y=mtxY, lambda=1
                )
     )
...
While the CG 150 is driving to 96.3% accuracy (2.5% higher classification error than BFGS 150).  And, we check that the starting cost is the same as the base case:  
```{r}
vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))  # Roll together bpThetaOne and bpThetaTwo
nnCost(vecMatrix=vecPass, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)
```
  
Additionally, we look at the CG 50 outcomes:  
```{r}
keyCut <- (nInputLayer + 1) * nHiddenNodes
postBPThetaOne <- matrix(data=nnListCG050$par[1:keyCut], nrow=nHiddenNodes, byrow=FALSE)
postBPThetaTwo <- matrix(data=nnListCG050$par[(keyCut+1):length(nnListCG050$par)], 
                         nrow=nFinalLabels, byrow=FALSE
                         )

# Assess the optimized accuracies
predYCG050 <- forwardPredY(mtxX, postBPThetaOne, postBPThetaTwo)
plot(x=1:length(predYCG050), y=predYCG050, col = 1 + (mtxY[,1] != predYCG050), 
     main="Predictions vs Actual (CG 50)", ylab="Prediction (Red=Wrong)", xlab="Index"
     )
confusionMatrix(predYCG050, mtxY[,1])

# Print the post-optimization cost
nnCost(vecMatrix=nnListCG050$par, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)

# Plot the post-optimization gradients
plot(nnGradient(vecMatrix=nnListCG050$par, nInputLayer, nHiddenNodes, nFinalLabels, 
                X=mtxX, y=mtxY, lambda=1
                )
     )
...
Accuracy here is only 89.5%, suggestive that 50 iterations of CG in R is driving to a less optimal outcome than 50 iterations of CG in Octave (which hits ~95%).  

Finally, we look at the CG 300 outcomes:  
```{r}
keyCut <- (nInputLayer + 1) * nHiddenNodes
postBPThetaOne <- matrix(data=nnListCG300$par[1:keyCut], nrow=nHiddenNodes, byrow=FALSE)
postBPThetaTwo <- matrix(data=nnListCG300$par[(keyCut+1):length(nnListCG300$par)], 
                         nrow=nFinalLabels, byrow=FALSE
                         )

# Assess the optimized accuracies
predYCG300 <- forwardPredY(mtxX, postBPThetaOne, postBPThetaTwo)
plot(x=1:length(predYCG300), y=predYCG300, col = 1 + (mtxY[,1] != predYCG300), 
     main="Predictions vs Actual (CG 300)", ylab="Prediction (Red=Wrong)", xlab="Index"
     )
confusionMatrix(predYCG300, mtxY[,1])

# Print the post-optimization cost
nnCost(vecMatrix=nnListCG300$par, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)

# Plot the post-optimization gradients
plot(nnGradient(vecMatrix=nnListCG300$par, nInputLayer, nHiddenNodes, nFinalLabels, 
                X=mtxX, y=mtxY, lambda=1
                )
     )
...
With CG 300, accuracy has improved to ~98.6%, within spitting distance of the BFGS 150 accuracy (98.8%).  However, the cost is still relatively higher, likely due to achieving this accuracy with larger thetas than the BFGS algorithm.  
  
Perhaps this continues to be an area for further exploration.  
  
###_Gradient Descent with Large Datasets_  
With much more data available, it is possible to build low bias models that are also low variance.  It is again "he who has the best data that wins".  But, working with the very largest data requires different techniques:  
  
* Suppose that you have 100 million observations (m=100 million)  
* This would mean that you need to sum across 100 million observations to find the gradient  
* A good idea is to first look at the learning curves for a smaller sample - if it shows high variance, more data may help, but if it shows high bias then more data will likely not help  
  
If the full 100 million record dataset appears to be useful, then there are techniques for running gradient descent more quickly.  One example is stochastic gradient descent:  
  
* Randomly shuffle the dataset (in case there is some structure)  
* Repeatedly run i=1 to m, and calculate theta(j) = theta(j) - alpha * { ( h(x(i)) - y(i) ) * x(j)(i) }  
* This means theta is being updated every time we see a data point; in fact, sometimes a single pass through the descent gets you to a good point  
* The main downside is that it means there is a meandering path to the global minima, which is never actually hit; however, the descent tends to find a "good enough" solution  
  
Another example approach is mini-batch, where b=10 is common, and b is very often in the range of 2-100.  The mini-batch approach uses b data points at once and alpha/b, meaning that 1) it can be vectorized, but 2) it requires an additional parameter.  If the code for mini-batch is not vectorized, then there is no real gain.  
  
Plotting cost curves is recommended with stochastic gradient descent or mini-batch gradient descent.  Basically, plot the cost after every 1,000 iterations and see how well it is converging.  If the cost is generally growing or oscillating too wildly, that may signal that alpha is too large.  
  
Let's try an example of the mini-batch approach for the neural network problem run above:  
```{r, cache=TRUE}
# Re-initialize vecPass to the beginning values
vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))  # Roll together bpThetaOne and bpThetaTwo

# Use a batch size of 50
nBatch <- 50

# What percentage of the data will be passed each time?  
# Need to scale lambda by this while running mini-batch gradients
keyRat <- nBatch / nrow(mtxX)


# Calculate the starting cost (full dataset)
startCost <- nnCost(vecMatrix=vecPass, nInputLayer, nHiddenNodes, nFinalLabels, 
                    X=mtxX, y=mtxY, lambda=1
                    )

# Set vecUse as vecPass and alpha=1 (rather large)
vecUse <- vecPass
alpha <- 1

# Set the number of runs and the timing for plotting the learning curve
nRuns <- 5000
nPerSave <- 1000
nPerSave <- nBatch * ceiling(nPerSave/nBatch)
costSave <- numeric(length=nRuns*nBatch/nPerSave)

startTime <- proc.time()

# Run through 5000 mini-batch descents
# Since the gradients already come back with 1/m, do not divide alpha by nBatch
for (intCtr in 1:nRuns) {
    miniRand <- sample(1:nrow(mtxX), nBatch, replace=FALSE)
    
    # Pass a smaller lambda to nnGradient to not over-penalize large thetas
    vecUse <- vecUse - (alpha) * nnGradient(vecMatrix=vecUse, nInputLayer, nHiddenNodes, 
                                            nFinalLabels, X=mtxX[miniRand, ], 
                                            y=mtxY[miniRand, ,drop=FALSE], lambda=1*keyRat
                                            )
    
    # Calculate actual costs based on all data (unscaled lambda)
    if ((intCtr * nBatch) %% nPerSave == 0) {
        costSave[[(intCtr * nBatch) %/% nPerSave]] <- nnCost(vecMatrix=vecUse, nInputLayer, 
                                                             nHiddenNodes, nFinalLabels, 
                                                             X=mtxX, y=mtxY, lambda=1
                                                             )
    }
...
plot(x=0:length(costSave), y=c(startCost, costSave), pch=19, col="blue", cex=0.5,
     ylim=c(0, max(startCost, costSave)), xlab="Sample Number", ylab="Total Cost",
     main="Mini-Batch Descent for Neural Network"
     )
abline(h=0.365, lty=2, lwd=2)

# Assess the optimized accuracies
keyCut <- (nInputLayer + 1) * nHiddenNodes
postBPThetaOne <- matrix(data=vecUse[1:keyCut], nrow=nHiddenNodes, byrow=FALSE)
postBPThetaTwo <- matrix(data=vecUse[(keyCut+1):length(vecUse)], 
                         nrow=nFinalLabels, byrow=FALSE
                         )

predYMini <- forwardPredY(mtxX, postBPThetaOne, postBPThetaTwo)
plot(x=1:length(predYMini), y=predYMini, col = 1 + (mtxY[,1] != predYMini), 
     main="Predictions vs Actual (Mini-Batch)", ylab="Prediction (Red=Wrong)", xlab="Index"
     )
confusionMatrix(predYMini, mtxY[,1])

plot(nnGradient(vecMatrix=vecUse, nInputLayer, nHiddenNodes, nFinalLabels, 
                X=mtxX, y=mtxY, lambda=1
                )
     )

# Comparison to the BFGS 150, CG 300, CG 150, and CG 50 algorithms
nnCost(vecMatrix=vecUse, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)
nnCost(vecMatrix=nnListBFGS150$par, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)
nnCost(vecMatrix=nnListCG300$par, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)
nnCost(vecMatrix=nnListCG150$par, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)
nnCost(vecMatrix=nnListCG050$par, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)
...
sum(vecUse^2)
sum((nnListBFGS150$par)^2)
sum((nnListCG300$par)^2)
sum((nnListCG150$par)^2)
sum((nnListCG050$par)^2)
```
  
So, the mini-batch (5000 batches of 50, or 50 times through the data in total) asymptotes to roughly the same cost as the BFGS 150 optimization, and is essentially at the asymptote by indices 50-100 (having been through the data 10-20 times in total).  Importantly, the mini-batch run takes ~40 seconds to get through the data 50 times, in contrast to the ~20 seconds that optim(method="CG") requires to get through the data 50 times.  However, the mini-batch approach has converged nicely well before the end of its 40 seconds, while the CG approach still has a bit more convergence to do by 150 iterations.  And, the CG approach is in the range of large thetas, so it is achieving its accuracy in a more risky manner.  

Careful selection of alpha and lambda play a large role in the convergence, demonstrating how valuable a hold-out sample for tuning key parameters can be.  
  
###_Holdout Sample for Mini-Batch_  
The below code implements a brief hold-out example for mini-batch, exploring the predictive power on data the model has not yet seen:  
```{r}

testMiniBatch <- function(vecUse=vecPass, nInputLayer=nInputLayer, nHiddenNodes=nHiddenNodes, 
                          nFinalLabels=nFinalLabels, trainX, trainY, cvX, cvY, useSeed=NULL,
                          useLambda=1, useAlpha=1, nBatch=50, nRuns=5000, nPerSave=1000, cmFull=TRUE) {
    
    # Calculate the starting costs for train and test
    startCostTrain <- nnCost(vecMatrix=vecUse, nInputLayer, nHiddenNodes, nFinalLabels, 
                             X=trainX, y=trainY, lambda=useLambda
                             )
    startCostTest <- nnCost(vecMatrix=vecUse, nInputLayer, nHiddenNodes, nFinalLabels, 
                            X=cvX, y=cvY, lambda=useLambda
                            )
    
    # Set the number of runs and the timing for plotting the learning curve
    keyRat <- nBatch / nrow(trainX)
    nPerSave <- nBatch * ceiling(nPerSave/nBatch)
    costTrainSave <- numeric(length=nRuns*nBatch/nPerSave)
    costTestSave <- numeric(length=nRuns*nBatch/nPerSave)

    # Use the random seed if it has been provided, so the sample goes in the same order
    # Note that sample(a, b) returns different numbers than sample(a, b/2) followed by sample(a, b/2)
    if (length(useSeed) > 0) {
        print(paste0("Setting seed to: ", useSeed))
        set.seed(useSeed)
    }
    
    startTime <- proc.time()

    # Run through mini-batch routine for train data only
    # Since gradients already come back with 1/m, do not divide useAlpha by nBatch
    for (intCtr in 1:nRuns) {
        miniRand <- sample(1:nrow(trainX), nBatch, replace=FALSE)
    
        # Pass a smaller lambda to nnGradient to not over-penalize large thetas
        vecUse <- vecUse - (useAlpha) * nnGradient(vecMatrix=vecUse, nInputLayer, nHiddenNodes, 
                                                   nFinalLabels, X=trainX[miniRand, ], 
                                                   y=trainY[miniRand, ,drop=FALSE], 
                                                   lambda=useLambda*keyRat
                                            )
    
        # Calculate actual costs based on all data (unscaled lambda)
        if ((intCtr * nBatch) %% nPerSave == 0) {
            costTrainSave[[(intCtr * nBatch) %/% nPerSave]] <- nnCost(vecMatrix=vecUse, nInputLayer,
                                                                      nHiddenNodes, nFinalLabels, 
                                                                      X=trainX, y=trainY, lambda=useLambda
                                                                 )
            costTestSave[[(intCtr * nBatch) %/% nPerSave]] <- nnCost(vecMatrix=vecUse, nInputLayer,
                                                                     nHiddenNodes, nFinalLabels, 
                                                                     X=cvX, y=cvY, lambda=useLambda
                                                                 )
        }
    ...
    plot(x=0:length(costTrainSave), y=c(startCostTrain, costTrainSave), pch=19, col="blue", 
         cex=0.5, ylim=c(0, max(startCostTrain, startCostTest, costTrainSave, costTestSave)), 
         xlab="Sample Number", ylab="Total Cost", 
         main=paste0("Mini-Batch - Lambda: ", round(useLambda,4), " Alpha: ", 
                     round(useAlpha,4), " Batch Size: ", nBatch
                     )
         )
    points(x=0:length(costTestSave), y=c(startCostTest, costTestSave), pch=19, col="red", cex=0.5)
    legend("topright", legend=c("Train", "Test"), pch=19, cex=1, col=c("blue", "red"))
    
    # Assess optimized accuracies
    keyCut <- (nInputLayer + 1) * nHiddenNodes
    postBPThetaOne <- matrix(data=vecUse[1:keyCut], nrow=nHiddenNodes, byrow=FALSE)
    postBPThetaTwo <- matrix(data=vecUse[(keyCut+1):length(vecUse)], 
                             nrow=nFinalLabels, byrow=FALSE
    ...
    predYTrainMini <- forwardPredY(trainX, postBPThetaOne, postBPThetaTwo)
    predYTestMini <- forwardPredY(cvX, postBPThetaOne, postBPThetaTwo)

    par(mfrow=c(1,2))
    
    plot(x=1:length(predYTrainMini), y=predYTrainMini, col = 1 + (trainY[,1] != predYTrainMini), 
         main="Predictions vs Actual (Train)", ylab="Prediction (Red=Wrong)", xlab="Index"
         )
    
    plot(x=1:length(predYTestMini), y=predYTestMini, col = 1 + (cvY[,1] != predYTestMini), 
         main="Predictions vs Actual (Test)", ylab="Prediction (Red=Wrong)", xlab="Index"
         )
    
    par(mfrow=c(1,1))
    
    if (cmFull) {
        print(confusionMatrix(predYTrainMini, trainY[,1]))
        print(confusionMatrix(predYTestMini, cvY[,1]))
    } else {
        print(confusionMatrix(predYTrainMini, trainY[,1])$overall)
        print(confusionMatrix(predYTestMini, cvY[,1])$overall)
    }
...
idxTrain <- sample(1:nrow(mtxX), .7*nrow(mtxX), replace=FALSE)
idxTrain <- idxTrain[order(idxTrain)]

testMiniBatch(vecUse=vecPass, nInputLayer=nInputLayer, nHiddenNodes=nHiddenNodes, 
              nFinalLabels=nFinalLabels, trainX=mtxX[idxTrain, ], trainY=mtxY[idxTrain, ,drop=FALSE], 
              cvX=mtxX[-idxTrain, ], cvY=mtxY[-idxTrain, ,drop=FALSE],
              useLambda=1, useAlpha=1, nBatch=50, nRuns=2000, nPerSave=1000
              )

```
  
In this case, it appears that the model may be over-fitting, as Train accuracy is ~97% while CV accuracy is ~93%.  This may mean that we need a larger lambda to further penalize the parameters, or more data.  
  
Next, we investigate the performance and accuracy impact of changing the mini-batch size:  
```{r, cache=TRUE}

for (batchSize in c(2, 5, 10, 25, 50, 100, 500)) {
    
    vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))
    
    # Each run will cycle through the data (3500 records) 10 times
    testMiniBatch(vecUse=vecPass, nInputLayer=nInputLayer, nHiddenNodes=nHiddenNodes, 
                  nFinalLabels=nFinalLabels, trainX=mtxX[idxTrain, ], trainY=mtxY[idxTrain, ,drop=FALSE], 
                  cvX=mtxX[-idxTrain, ], cvY=mtxY[-idxTrain, ,drop=FALSE],
                  useLambda=1, useAlpha=1, nBatch=batchSize, 
                  nRuns=ceiling(35000/batchSize), nPerSave=1000, cmFull=FALSE
                  )
...
There is a time penalty for the small (n=2-5) batch size relative to the medium/large (n=10+) batches, likely due to the speed advantages of vectorization.  Further, the small batch size struggles to converge, suggesting that a smaller alpha would likely be needed if such a small batch size will be maintained.  

Further, as the batch size gets "too small" or "too large", out-of-sample accuracy appears to get slightly worse.  With the small batch sizes, there seems to be oscillation, suggestive that alpha is too large.  With the large batch sizes, there seems to be room still to optimize, suggestive that alpha is too small.  Perhaps there is an optimal alpha per batch size.  We explore modifying alpha for batch sizes of 10:  
```{r, cache=TRUE}

for (alphaSize in c(0.1, 0.3, 1, 3, 10)) {
    ...
    testMiniBatch(vecUse=vecPass, nInputLayer=nInputLayer, nHiddenNodes=nHiddenNodes, 
                  nFinalLabels=nFinalLabels, trainX=mtxX[idxTrain, ], trainY=mtxY[idxTrain, ,drop=FALSE], 
                  cvX=mtxX[-idxTrain, ], cvY=mtxY[-idxTrain, ,drop=FALSE],
                  useLambda=1, useAlpha=alphaSize, nBatch=10, 
                  nRuns=3500, nPerSave=1000, cmFull=FALSE
                  )
...
In this case, it appears that alpha ~0.1 does not allow the solution to converge in just 10 runs, while alpha ~3+ creates oscillations and an inferior solution.  

The continued existence of a cost/accuracy gap between the training and CV data is suggestive of over-fitting.  One potential cure for that is to set a higher lambda, driving "worse" fits for the training data, but possibly improving the CV error by reducing training on noise.  These solutions use a batch size of 10 with a learning rate alpha=1:  
```{r, cache=TRUE}

for (lambdaSize in c(0.1, 0.25, 0.5, 1, 2, 4, 10)) {
    ...
    testMiniBatch(vecUse=vecPass, nInputLayer=nInputLayer, nHiddenNodes=nHiddenNodes, 
                  nFinalLabels=nFinalLabels, trainX=mtxX[idxTrain, ], trainY=mtxY[idxTrain, ,drop=FALSE], 
                  cvX=mtxX[-idxTrain, ], cvY=mtxY[-idxTrain, ,drop=FALSE],
                  useLambda=lambdaSize, useAlpha=1, nBatch=10, 
                  nRuns=3500, nPerSave=1000, cmFull=FALSE
                  )
...
Lambda appears to be an interesting double-edged sword.  Smaller lambdas allow for a better fit (higher Train accuracy) and also a larger over-fit (differences between Train and CV accuracy).  Out-of-sample accuracy appears reasonably stable across lambdas given the other parameters in this mini-batch.
  
Lastly, we do a bit of space exploration using lambda of 1:  
```{r, cache=TRUE}
for (alphaSize in c(0.3, 0.6, 1, 1.5)) {
    for (batchSize in c(10, 25, 50, 100)) {
        
        vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))
    
        # Each run will cycle through the data (3500 records) 10 times
        testMiniBatch(vecUse=vecPass, nInputLayer=nInputLayer, nHiddenNodes=nHiddenNodes, 
                      nFinalLabels=nFinalLabels, trainX=mtxX[idxTrain, ], 
                      trainY=mtxY[idxTrain, ,drop=FALSE], 
                      cvX=mtxX[-idxTrain, ], cvY=mtxY[-idxTrain, ,drop=FALSE],
                      useLambda=1, useAlpha=alphaSize, nBatch=batchSize, 
                      nRuns=35000/batchSize, nPerSave=1000, cmFull=FALSE, useSeed=1606131243
                      )
...
In general, it seems that a larger batch size works better with a slightly larger alpha.  This may be due to a more stable average gradient, meaning you want to move farther in each step if you are only going to cycle through the data 10 times.  It is interesting how quickly the mini-batch gets to a reasonable solution.  If you can drive to 90%-95% accuracy in 10 seconds, that can also give you the starting thetas for BFGS or CG or the like in the search for the final 5%-10%.  
