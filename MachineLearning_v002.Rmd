...
Still, it provides a good starting solution to a neural network with ~10,000 parameters for ~50,000 predictions (10 final nodes for each of the 5,000 observations) in ~5 minutes.  So, while it can likely be significantly further optimized, it is pretty cool to be able to do simple image identification (on a cleaned dataset consisting of numbers only) in not too much time.  
...
* 10 runs of the cost function take ~0.5 seconds  
* 10 runs of the gradient function take ~1.1 seconds  
* Total cost after 10 runs of BFGS is ~160k, roughly double the ~80k seen above at 50 iterations  
* 43 runs of cost and 10 runs of gradient made by way of BFGS in optim() take ~140 seconds  
* Extrapolating, the raw functions should take 4.3 * 0.5 + 1.1 = ~3.5 seconds  
...
* Total cost after 10 runs of CG is ~45k, roughly half the ~80k seen above at 50 iterations of BFGS  
* 28 runs of cost and 11 runs of gradient made by way of BFGS in optim() take ~3 seconds  
* Extrapolating, the raw functions should take 2.8 * 0.5 + 1.1 = ~2.5 seconds  
...
It would also be interesting to understand whether method="CG" could help with the neural network tuned previously.  The same code is run, with the method changed to CG for 10 iterations, 50 iterations, and 150 iterations.  The goal is to see whether the CG run-times and/or accuracies can be improved:  
```{r, cache=TRUE}
...
* 10 runs of nnGradient takes ~1.5 seconds  
* Running optim() using BFGS for 150 iterations takes ~240 seconds, while making 257 calls to nnCost and 150 calls to nnGradient; these should only take ~50 seconds, so optim() is taking ~5x as long as expected  
...
* Running optim() using CG for 50 iterations takes ~17 seconds, while making 106 calls to nnCost and 51 calls to nnGradient; these should take ~19 seconds, suggesting very little inefficiency  
* Running optim() using CG for 150 iterations takes ~50 seconds, while making 307 calls to nnCost and 151 calls to nnGradient; these should take ~55 seconds, suggesting very little inefficiency  
  
Additionally, the results are again plotted to see the estimated cost rates:  
```{r}
nnCostValues <- c(nnListBFGS150$value, nnListCG010$value, nnListCG050$value, nnListCG150$value)
...
axis(1, at=1:4, labels=c("BFGS 150 iter", "CG 10 iter", "CG 50 iter", "CG 150 iter"))

```
  
Interestingly, though, in this case CG is not converging as quickly to a good solution as had been observed in the BFGS modeling.  So, the algorithm may be faster but less accurate in this case.  
...
So, the BFGS 150 is driving 98.2% accuracy.  Next, we look at the CG 150 outcomes:  
```{r}
keyCut <- (nInputLayer + 1) * nHiddenNodes
...
While the CG 150 is driving to 97.8% accuracy (0.4% higher classification error than BFGS 150).  And, we check that the starting cost is the same as the base case:  
```{r}
vecPass=c(as.vector(bpThetaOne), as.vector(bpThetaTwo))  # Roll together bpThetaOne and bpThetaTwo
nnCost(vecMatrix=vecPass, nInputLayer, nHiddenNodes, nFinalLabels, X=mtxX, y=mtxY, lambda=1)
```
  
Lastly, we look at the CG 50 outcomes:  
```{r}
keyCut <- (nInputLayer + 1) * nHiddenNodes
...
And, accuracy here is only 89.5%, suggestive that 50 iterations of CG in R is driving to a less optimal outcome than 50 iterations of CG in Octave (which hits ~95%).  So, perhaps there is still further tuning to explore?  
...
# Use an adjusted lambda in functions that take lambda/m
...
# Run through 1000 mini-batch descents
...
for (intCtr in 1:nRuns) {
    ...
    vecUse <- vecUse - (alpha) * nnGradient(vecMatrix=vecUse, nInputLayer, nHiddenNodes, 
                                            ...
                                            y=mtxY[miniRand, ,drop=FALSE], lambda=1
                                            )
...
plot(x=0:length(costSave), y=c(startCost, costSave), pch=19, col="blue", 
...
# Comparison to the BFGS 150 algorithm
...
So, the mini-batch converges to a lower overall cost mainly by keeping theta smaller (and avoiding that penalty) even though it means keeping the accuracy lower also.
  
* NEED TO FIX nnGradient to use LAMBDA  
* NEED TO INVESTIGATE how to better use LAMBDA in the later functions  
